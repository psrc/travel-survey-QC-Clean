...
GO
	...
	AS
	SELECT t1.recid, t1.person_id AS person_id, t1.hhid, t1.pernum, CASE WHEN h.hhgroup=11 THEN 'rMove' ELSE 'rSurvey' END AS hhgroup, 
...
-- All-inclusive person-level views for FixieUI (only for reference)
		...
		SELECT p.person_id AS person_id, p.hhid AS hhid, p.pernum, ac.agedesc AS Age, 
...
-- Person-level views for FixieUI Main forms, separated by staff so as to avoid editing conflicts
		...
		DROP VIEW IF EXISTS HHSurvey.Person_Suzanne
		...
		FROM HHSurvey.Person AS p INNER JOIN HHSurvey.AgeCategories AS ac ON p.age_detailed = ac.agecode JOIN HHSurvey.Household AS h on h.hhid=p.hhid
		WHERE EXISTS (SELECT 1 FROM HHSurvey.trip_error_flags AS tef WHERE p.person_id = tef.person_id) --AND (tef.error_flag IN('excessive speed','no activity time after','too slow','instantaneous'))) --'non-student + school trip',
		--OR EXISTS (SELECT 1 FROM HHSurvey.Trip AS tm WHERE p.person_id = tm.person_id AND (tm.psrc_comment LIKE 'UNLIN%' OR tm.psrc_comment LIKE 'INSER%'))
		;
		GO
		...
		WHERE EXISTS (SELECT 1 FROM HHSurvey.trip_error_flags AS tefx WHERE p.person_id = tefx.person_id) AND 
		      EXISTS (SELECT 1 FROM HHSurvey.Trip AS te WHERE p.person_id = te.person_id AND te.psrc_comment IS NOT NULL AND te.psrc_comment<>'' ) 
		...
		SELECT TOP 7 p.person_id AS person_id, p.hhid AS hhid, p.pernum, ac.agedesc AS Age, 
			CASE WHEN p.employment BETWEEN 1 AND 4 THEN 'Yes' ELSE 'No' END AS Works, 
			CASE WHEN p.student = 1 THEN 'No' WHEN student IN(2,4,5) THEN 'PT' WHEN p.student IN(3,6,7) THEN 'FT' ELSE 'No' END AS Studies, 
			CASE WHEN h.hhgroup = 11 THEN 'rMove' WHEN h.hhgroup IS NOT NULL THEN 'rSurvey' ELSE 'n/a' END AS HHGroup
		FROM HHSurvey.Person AS p INNER JOIN HHSurvey.AgeCategories AS ac ON p.age_detailed = ac.agecode JOIN HHSurvey.Household AS h on h.hhid=p.hhid
		WHERE Exists (SELECT 1 FROM HHSurvey.trip_error_flags AS tef WHERE p.person_id = tef.person_id)-- AND tef.error_flag IN('change mode purpose','too long at dest?'))
...
/*		CREATE VIEW HHSurvey.person_Polina WITH SCHEMABINDING AS
		...
		SELECT p.person_id AS person_id, p.hhid AS hhid, p.pernum, ac.agedesc AS Age, 
			...
			CASE WHEN h.hhgroup = 11 THEN 'rMove' WHEN h.hhgroup IS NOT NULL THEN 'rSurvey' ELSE 'n/a' END AS HHGroup
		FROM HHSurvey.Person AS p INNER JOIN HHSurvey.AgeCategories AS ac ON p.age_detailed = ac.agecode JOIN HHSurvey.Household AS h on h.hhid=p.hhid
		WHERE Exists (SELECT 1 FROM HHSurvey.trip_error_flags AS tef WHERE tef.person_id = p.person_id AND tef.error_flag IN('missing next trip link'))
			AND NOT EXISTS (SELECT 1 FROM cte WHERE cte.person_id=p.person_id);
		GO
		...
		SELECT p.person_id AS person_id, p.hhid AS hhid, p.pernum, ac.agedesc AS Age, 
			...
			CASE WHEN h.hhgroup = 11 THEN 'rMove' WHEN h.hhgroup IS NOT NULL THEN 'rSurvey' ELSE 'n/a' END AS HHGroup
		FROM HHSurvey.Person AS p INNER JOIN HHSurvey.AgeCategories AS ac ON p.age_detailed = ac.agecode JOIN HHSurvey.Household AS h on h.hhid=p.hhid
		WHERE Exists (SELECT 1 FROM HHSurvey.trip_error_flags AS tef WHERE tef.person_id = p.person_id AND tef.error_flag IN(''))
		AND NOT EXISTS (SELECT 1 FROM cte WHERE cte.person_id = p.person_id);
		GO

		CREATE VIEW HHSurvey.person_Suzanne WITH SCHEMABINDING AS
		WITH cte AS (SELECT person_id FROM HHSurvey.person_Mike 
		   UNION ALL SELECT person_id FROM HHSurvey.person_Elevated 
		   UNION ALL SELECT person_id FROM HHSurvey.person_Parastoo
		   UNION ALL SELECT person_id FROM HHSurvey.person_Polina
		...
		WITH cte AS (SELECT person_id FROM HHSurvey.person_Mike 
		   ...
		   UNION ALL SELECT person_id FROM HHSurvey.person_Polina
		   UNION ALL SELECT person_id FROM HHSurvey.person_Mary
		   UNION ALL SELECT person_id FROM HHSurvey.person_Suzanne
		   )
		SELECT p.person_id AS person_id, p.hhid AS hhid, p.pernum, ac.agedesc AS Age, 
			CASE WHEN p.employment BETWEEN 1 AND 4 THEN 'Yes' ELSE 'No' END AS Works,  
			CASE WHEN p.student = 1 THEN 'No' WHEN student IN(2,4,5) THEN 'PT' WHEN p.student IN(3,6,7) THEN 'FT' ELSE 'No' END AS Studies, 
		...
		WITH cte AS (SELECT person_id FROM HHSurvey.person_Mike 
		   ...
		   --UNION ALL SELECT person_id FROM HHSurvey.person_Mary
		   --UNION ALL SELECT person_id FROM HHSurvey.person_Suzanne
		   --UNION ALL SELECT person_id FROM HHSurvey.person_Abdi
		   )
		SELECT p.person_id AS person_id, p.hhid AS hhid, p.pernum, ac.agedesc AS Age, 
			CASE WHEN p.employment BETWEEN 1 AND 4 THEN 'Yes' ELSE 'No' END AS Works, 
			CASE WHEN p.student = 1 THEN 'No' WHEN student IN(2,4,5) THEN 'PT' WHEN p.student IN(3,6,7) THEN 'FT' ELSE 'No' END AS Studies, 
			CASE WHEN h.hhgroup = 11 THEN 'rMove' WHEN h.hhgroup IS NOT NULL THEN 'rSurvey' ELSE 'n/a' END AS HHGroup
		FROM HHSurvey.Person AS p INNER JOIN HHSurvey.AgeCategories AS ac ON p.age_detailed = ac.agecode JOIN HHSurvey.Household AS h on h.hhid=p.hhid
		WHERE Exists (SELECT 1 FROM HHSurvey.trip_error_flags AS tef WHERE tef.person_id = p.person_id)-- AND tef.error_flag IN('mode_1 missing')
		AND NOT EXISTS (SELECT 1 FROM cte WHERE cte.person_id = p.person_id);
		GO
/*
SELECT count(*) AS n,'Grant' AS form FROM HHSurvey.person_Grant
--UNION ALL select count(*),'Abdi' FROM HHSurvey.person_Abdi
--UNION ALL select count(*),'Suzanne' FROM HHSurvey.person_Suzanne
